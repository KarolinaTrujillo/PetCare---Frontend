    ...
    <div style={{ position: "fixed", inset: 0, zIndex: 9999, display: "flex", alignItems: "center", justifyContent: "center", backgroundColor: "rgba(0,0,0,0.42)", backdropFilter: "blur(3px)" }}>
      <div style={{ backgroundColor: C.white, borderRadius: "18px", width: "100%", maxWidth: "440px", padding: "36px 36px 28px", boxShadow: "0 20px 48px rgba(0,0,0,0.18)", boxSizing: "border-box" }}>
        ...
        <div style={{ marginBottom: "24px" }}>
          ...
          <p style={{ fontSize: "13px", color: C.textSub }}>Ingresa y confirma tu nueva contraseña.</p>
        </div>

        <div style={{ marginBottom: "16px" }}>
          <label style={{ fontSize: "13px", fontWeight: 600, color: C.textSub, display: "block", marginBottom: "6px" }}>Nueva contraseña</label>
          <div style={{ position: "relative" }}>
            ...
            <button type="button" onClick={() => setShowNew((v) => !v)} style={{ position: "absolute", right: "10px", top: "50%", transform: "translateY(-50%)", background: "none", border: "none", cursor: "pointer", padding: "2px", display: "flex" }}>
        ...
        <div style={{ marginBottom: "20px" }}>
          <label style={{ fontSize: "13px", fontWeight: 600, color: C.textSub, display: "block", marginBottom: "6px" }}>Confirmar contraseña</label>
          <div style={{ position: "relative" }}>
            <input type={showConfirm ? "text" : "password"} value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)}
              style={{ width: "100%", boxSizing: "border-box", padding: "10px 40px 10px 14px", borderRadius: "8px", border: `1px solid ${C.border}`, fontSize: "14px", color: C.textMain, outline: "none" }} />
            <button type="button" onClick={() => setShowConfirm((v) => !v)} style={{ position: "absolute", right: "10px", top: "50%", transform: "translateY(-50%)", background: "none", border: "none", cursor: "pointer", padding: "2px", display: "flex" }}>
        ...
        <div style={{ display: "flex", justifyContent: "flex-end", gap: "12px" }}>
          <button onClick={onClose} style={{ background: "none", border: `1px solid ${C.border}`, borderRadius: "8px", padding: "9px 18px", fontSize: "14px", fontWeight: 500, color: C.textSub, cursor: "pointer" }}
            onMouseEnter={(e) => { (e.currentTarget as HTMLButtonElement).style.borderColor = C.textSub; }}
            onMouseLeave={(e) => { (e.currentTarget as HTMLButtonElement).style.borderColor = C.border; }}>
            Cancelar
          </button>
          <button onClick={onSubmit} disabled={passwordSaving} style={{ backgroundColor: passwordSaving ? "#9ECEC6" : C.green, color: C.white, border: "none", borderRadius: "8px", padding: "9px 20px", fontSize: "14px", fontWeight: 700, cursor: passwordSaving ? "not-allowed" : "pointer" }}
