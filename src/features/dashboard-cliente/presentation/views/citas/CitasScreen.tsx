...
export const CitasScreen = () => {
  ...
  return (
    <div className="h-screen flex flex-col overflow-hidden">
      <NavBarComponent title="CITAS" subtitle="Gestiona tus citas aquí..." />
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2 md:gap-4">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-black font-semibold text-base md:text-lg">Mis citas</h2>
            </div>
            <span className="text-xs md:text-sm text-gray-400">{citasFiltradas.length} resultado{citasFiltradas.length !== 1 ? 's' : ''}</span>
          </div>
          <button
            ...
            {estadoOpciones.map(op => (
              <button
                ...
                className={`text-xs font-semibold px-3 py-1.5 rounded-full transition-colors cursor-pointer ${
                  filtro === op ? estadoStyles[op] : 'bg-gray-100 text-gray-400 hover:bg-gray-200'
                }`}
              >
                {op}
              </button>
        ...
        {citasFiltradas.length > 0 ? (
          ...
          <div className="relative flex-1 flex flex-col items-center overflow-hidden -mx-4 md:-mx-6 -mb-4 md:-mb-8">
            <div className="mt-8 md:mt-16 z-10 flex flex-col items-center gap-2 px-4 text-center">
              <p className="text-gray-900 text-xl md:text-3xl font-semibold">No tienes citas recientes</p>
