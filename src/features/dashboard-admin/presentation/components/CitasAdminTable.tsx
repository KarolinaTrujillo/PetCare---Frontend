...
export const CitasAdminTable = ({ citas }: { citas: CitaAdminProps[] }) => {
  return (
    <div className="bg-white border border-gray-100 rounded-2xl shadow-sm flex flex-col overflow-hidden">
      <div className="flex items-center justify-between px-5 py-4 border-b border-gray-50">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-sm font-bold text-gray-900">Citas recientes</h2>
        </div>
        <Link href={Routes.dashboard.admin.citas} className="text-xs text-[#267A6E] hover:underline flex items-center gap-1">
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="text-xs text-gray-400 uppercase tracking-wider border-b border-gray-50">
              ...
              <th className="px-5 py-3 text-left font-semibold">Veterinario</th>
          ...
          <tbody>
            {citas.map((c, i) => (
              <tr key={c.id} className={`hover:bg-gray-50 transition-colors ${i !== citas.length - 1 ? 'border-b border-gray-50' : ''}`}>
                ...
                <td className="px-5 py-3 text-gray-500">{c.nombre_veterinario} {c.apellido_veterinario}</td>
